...
- **`db/schema/*.ts`**: One file per table, `pgTable("<snake_case_table>", { ... })`, exported as a `camelCase` plural const (e.g., `export const accounts = pgTable("accounts", {...})`). Primary keys are `text("id").primaryKey().$defaultFn(() => randomUUIDv7())`. Re-export every table from `db/schema/index.ts`. A column backed by a fixed set of string values uses a `pgEnum` whose values come from the matching `tp*Enum`/`st*Enum` (see section 9a) — never a bare inline array.
- **`db/migrations/`**: Generated by `drizzle-kit` (`bun run db:generate` / `db:migrate` / `db:push`) — do not hand-edit generated SQL. **Not committed to git while a phase's schema is still being iterated on** — `api/src/db/migrations` stays gitignored until the phase is ready to ship, at which point it's regenerated from a clean slate as a single migration and committed. Keep using it locally for `db:migrate`/`db:push` in the meantime.
...
- **`index.ts` barrels**: every `domain/repositories/`, `app/dtos/`, `app/schemas/`, and `app/usecases/` folder re-exports its contents from an `index.ts`. Import from the barrel (`../../domain/repositories`) from outside the folder, not the individual file.
- **Extract reusable functions** into a dedicated helper under `http/helpers/` (or a module-local `domain/services/` for framework-free logic, per section 3) instead of inlining them in a `.drizzle.ts` repository or usecase — especially anything more than one module needs.
- **Enums**: a fixed set of string values is a real TypeScript `enum` in its own file, never an inline string-literal union or a bare `z.enum([...])`/`pgEnum([...])` array — both derive their values from the enum. The prefix says what kind it is, and it's never repeated as a word in the name (the prefix already says it):
  - `tp` for a type/category, e.g. `tp-category.enum.ts` → `export enum tpCategoryEnum { INCOME = "income", EXPENSE = "expense" }` (not `tpCategoryTypeEnum`);
  ...
  - `lg` for a boolean-ish logical flag.
- **English only** for identifiers, file/folder names, and any (rare) comment. Nothing user-facing originates in this layer, so there's no Portuguese here at all — that only enters at the web layer's UI text.

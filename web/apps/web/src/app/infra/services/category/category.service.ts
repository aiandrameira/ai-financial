...
export class CategoryService implements CategoryRepository {
    ...
    create(body: RequestCategoryDto): Observable<CategoryDto> {
        return this.#client.post(this.#api, this.#toBody(body)).pipe(map(response => mapGet(response)));
    }

    update(id: string, body: RequestCategoryDto): Observable<void> {
        return this.#client.put<void>(`${this.#api}/${id}`, this.#toBody(body));
    }
    ...
    #toBody(body: RequestCategoryDto) {
